...
int main(void){
	...
	xil_printf("Current Settings are expecting a reference clock frequency of %d kHz.\r\n", REFCLK_FREQUENCY);
		...
		#ifdef SERIAL
			#ifdef SDCARD
				printf("The acquired image will be saved to SD Card and sent to Serial.\n\r");
			#else
				printf("The acquired image will be sent over Serial only.\n\r");
			#endif
		#elif defined(SDCARD)
			printf("The acquired image will be saved to SD Card only.\n\r");
		#else
			printf("either Serial or SD card should be selected in global_defines.h.\n\r");
			while(1);
		#endif
		//xil_printf("the acquired image will be saved on SD Card");
...
//    destination[7] = 0xDEADC0DE;
	...
	xil_printf("Acquiring Image took %d ms.\n\r", duration_ms);
	...
	//}
#ifdef SDCARD
	Status = ReportHistPerPixelSDCard(image_buffer, NUM_PIXELS, HISTOGRAM_BINS);
	if (Status != XST_SUCCESS) {
		xil_printf("Reporting hist to SDCard failed\r\n");
	}
	xil_printf("B 0\n\r");
	xil_printf("DONE\n\r");
#endif
	...
	if (Status != XST_SUCCESS) {
		xil_printf("Reporting hist to Serial failed\r\n");
	}
	xil_printf("DONE\n\r");
#endif
