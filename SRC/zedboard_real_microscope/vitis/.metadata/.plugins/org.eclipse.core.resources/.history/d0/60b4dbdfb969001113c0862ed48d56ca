...
*/
		...
		//axiGpOutClear(LD);
		for (int i = 0; i < 5; i++) {
		    if (CNTVALUEIN_CLK & (1 << i)) {
		        axiGpOutSet(CNTVALUEIN_CLK0 + i);
		    }
		}
		for (int i = 0; i < 5; i++) {
		    if (CNTVALUEIN_SDO & (1 << i)) {
		        axiGpOutSet(CNTVALUEIN_SDO0 + i);
		    }
		}
		for (int i = 0; i < 5; i++) {
		    if (CNTVALUEIN_FRAME & (1 << i)) {
		        axiGpOutSet(CNTVALUEIN_FRAME0 + i);
		    }
		}

		axiGpOutSet(IDELAYE2_LD);  // load the CNTVALUEIN into the idelay unit
...
*/
		...
		long int histogram_bins_min;
		if(HISTOGRAM_NUMBER_OF_BINS == 0){
			histogram_bins_min = (long int)ceil((double) laser_period_ps / (double) BIN_WIDTH);
		}
		else {
			histogram_bins_min = HISTOGRAM_NUMBER_OF_BINS;
		}
