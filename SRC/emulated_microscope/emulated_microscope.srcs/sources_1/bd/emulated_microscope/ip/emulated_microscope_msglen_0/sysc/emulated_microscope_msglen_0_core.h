...
#ifndef _emulated_microscope_msglen_0_core_h_
#define _emulated_microscope_msglen_0_core_h_

#include <systemc.h>
#include "properties.h"


class emulated_microscope_msglen_0_core : public sc_module 
{
    public: 
       emulated_microscope_msglen_0_core (sc_core::sc_module_name nm, const xsc::common_cpp::properties& props)
            : sc_module(nm)
              , Din    ( "Din" )
              , Dout   ( "Dout" )
    {
        SC_HAS_PROCESS(emulated_microscope_msglen_0_core);
        SC_METHOD(get_slice_value);
        sensitive << Din;

    }

    virtual ~emulated_microscope_msglen_0_core() = default;
