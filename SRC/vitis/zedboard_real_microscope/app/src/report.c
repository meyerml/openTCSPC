...
#define BUFFERSIZE ((1000000000/REFCLK_FREQUENCY)/(1<<LOG_2_HIST_BIN_WIDTH))*11
...
int ReportHistPerPixelSDCard(const u8 *global_destination, u32 num_pixels, u32 num_bins_per_hist){
	int Status;
	Status = SD_Init();
	...
	for (int pixel = 0; pixel<num_pixels; pixel++){
		ReportHistSDCard(pixel, buffer_address, num_bins_per_hist);
		buffer_address += (HISTOGRAM_BINS *2) + 8; //8 bytes of start signal
	}
	closeFile(fileptr);
	SD_Eject();
...
int ReportHistSDCard(int pixel, void* BufferAddress, int bins_per_hist){
	// write index of pixel
	//xil_printf("\n\r");
	char *dataPntrBegin = dataPntr;


	int chars_written = sprintf(dataPntr,"B %d\n", pixel);
	dataPntr = dataPntrBegin + chars_written;
	...
	for (int t = 0; t<bins_per_hist/4; t++) { //iterate over axi transactions of 64 bits = 4 bins
		chars_written += sprintf(dataPntr, "%d_", *(u16*)(BufferAddress+6));
		dataPntr = dataPntrBegin + chars_written;  //advance the sd buffer pointer as well
		chars_written += sprintf(dataPntr, "%d_", *(u16*)(BufferAddress+4));
		dataPntr = dataPntrBegin + chars_written;  //advance the sd buffer pointer as well
		chars_written += sprintf(dataPntr, "%d_", *(u16*)(BufferAddress+2));
		dataPntr = dataPntrBegin + chars_written;  //advance the sd buffer pointer as well
		chars_written += sprintf(dataPntr, "%d\n", *(u16*)(BufferAddress));
		dataPntr = dataPntrBegin + chars_written;  //advance the sd buffer pointer as well
	...
	//write end of pixel
	chars_written += sprintf(dataPntr, "E %d\n", pixel);
	dataPntr = dataPntrBegin + chars_written;



	//xil_printf("Updating SD card...\n\r");
	writeFile(fileptr, chars_written, (u32)dataBuffer);
